...
var URL = 'https://hs.boisestate.edu';
...
// Test that the left nav has a COLLEGE OF HEALTH SCIENCES HOME link, and that it's of the proper class
driver.findElements({id: 'menu-item-591'}).then(function(elements) {
        var isPresent = elements.length;
        if (isPresent){
                console.log('Page element of ID \'menu-item-591\' (the COLLEGE OF HEALTH SCIENCES HOME link) is here');
        } else {
                console.log('ERROR: Page element of class \'menu-item-591\' (the COLLEGE OF HEALTH SCIENCES HOME link) is NOT FOUND');
        }
});
...
// Test that a 'STUDENT SERVICES AND ACADEMIC ADVISING' item appears in the left nav.
driver.findElements({id: 'menu-item-7911'}).then(function(elements) {
        var isPresent = elements.length;
        if (isPresent){
                console.log('Page element \'menu-item-7911\' (\'STUDENT SERVICES AND ACADEMIC ADVISING left-nav link\')) is here');
                elements[0].getText().then(function (text) { console.log('\ttext = ' + text); });
        } else {
                console.log('ERROR: Page element \'menu-item-7911\' (\'STUDENT SERVICES AND ACADEMIC ADVISING left-nav link\')) is NOT FOUND');
        }
});
...
driver.findElements({className: 'post-footer'}).then(function(elements) {
        ...
        if (isPresent){
                ...
                require('./includes/footer.js').testPhoneNumber(webdriver, driver, '208-426-4141');
                require('./includes/footer.js').testDeptName(webdriver, driver, 'COLLEGE OF HEALTH SCIENCES');
                require('./includes/footer.js').testEmail(webdriver, driver, 'COHS@BOISESTATE.EDU');
                require('./includes/footer.js').testMailingAddress(webdriver, driver, 'ADMINISTRATIVE OFFICES IN NORCO SUITE 408');
        } else {
...
element.getText().then(function (text) { 
	...
	if (text == 'WELCOME TO COLLEGE OF HEALTH SCIENCES') {
		console.log('\tText is correct: ' + text);
	} else {
		console.log('ERROR: Page element \'entry-title\' text is incorrect.\n\tReads: ' + text + ' but should read WELCOME TO COLLEGE OF HEALTH SCIENCES');
	}
