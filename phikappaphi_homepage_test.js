...
var URL = 'https://phikappaphi.boisestate.edu';
...
// Test that the left nav has a WELCOME link, and that it has the proper id
driver.findElements({id: 'menu-item-761'}).then(function(elements) {
        var isPresent = elements.length;
        if (isPresent){
                console.log('Page element of ID \'menu-item-761\' (the WELCOME link) is here');
        } else {
                console.log('ERROR: Page element of class \'menu-item-761\' (the WELCOME link) is NOT FOUND');
        }
});
...
// Test that a SCHOLARSHIPS AND AWARDS item appears in the left nav.
driver.findElements({id: 'menu-item-741'}).then(function(elements) {
        var isPresent = elements.length;
        if (isPresent){
                console.log('Page element \'menu-item-741\' (\'SCHOLARSHIPS AND AWARDS left-nav link\') is here');
                elements[0].getText().then(function (text) { console.log('\ttext = ' + text); });
        } else {
                console.log('ERROR: Page element \'menu-item-741\' (\'SCHOLARSHIPS AND AWARDS left-nav link\') is NOT FOUND');
        }
});
...
driver.findElements({className: 'post-footer'}).then(function(elements) {
        ...
        if (isPresent){
                ...
                require('./includes/footer.js').testPhoneNumber(webdriver, driver, '426-7089');
                require('./includes/footer.js').testDeptName(webdriver, driver, 'PHI KAPPA PHI');
                require('./includes/footer.js').testEmail(webdriver, driver, 'RUSSELLWILLERTON@BOISESTATE.EDU');
        ...
        if (isPresent){
                ...
                elements[0].getText().then(function (text) {
                        if (text == 'WELCOME') {
                                ...
                                console.log('\t(Should read \'WELCOME\'');
                        }
                });
        } else {
