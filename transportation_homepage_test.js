...
var URL = 'https://transportation.boisestate.edu';
...
driver.findElements({className: 'post-footer'}).then(function(elements) {
	console.log('Checking that the post-footer is present and accurate...');
		...
		require('./includes/footer.js').testPhoneNumber(webdriver, driver, 'PHONE: 208-426-7275');
		require('./includes/footer.js').testDeptName(webdriver, driver, 'TRANSPORTATION & PARKING');
		require('./includes/footer.js').testEmail(webdriver, driver, 'PARKINGQUESTIONS@BOISESTATE.EDU');
		require('./includes/footer.js').testMailingAddress(webdriver, driver, 'STUDENT UNION BUILDING, TRANSIT CENTER 8AM-5PM M-F');
        } else {
...
driver.findElements({className: 'entry-title'}).then(function(elements) {
		...
		elements[0].getText().then(function (text) {
	                if (text == 'WELCOME TO THE TRANSPORTATION AND PARKING SERVICES WEBSITE!') {
       	                	 console.log('\tText is correct: ' + text);
                	} else {
                        	console.log('\tERROR: Text is INCORRECT: ' + text);
				console.log('\t(Should read \'WELCOME TO THE TRANSPORTATION AND PARKING SERVICES WEBSITE!\'');
                	}
		});
        } else {
