...
var URL = 'https://cobr.boisestate.edu';
...
driver.findElements({id: 'menu-item-71'}).then(function(elements) {
        ...
        if (isPresent){
                console.log('Page element of ID \'menu-item-71\' (the HOME link) is here');
        } else {
                console.log('ERROR: Page element of class \'menu-item-71\' (the HOME link) is NOT FOUND');
        }
});
...
driver.findElements({className: 'post-footer'}).then(function(elements) {
        ...
        if (isPresent){
                ...
                require('./includes/footer.js').testDeptName(webdriver, driver, 'CENTER FOR ORTHOPAEDIC & BIOMECHANICS RESEARCH');
                require('./includes/footer.js').testEmail(webdriver, driver, 'COBR@BOISESTATE.EDU');
                require('./includes/footer.js').testMailingAddress(webdriver, driver, 'RON & LINDA YANKE FAMILY RESEARCH PARK');
        } else {
