...
var URL = 'https://hs.boisestate.edu/radsci/';
...
// Test that a 'RADIOLOGY' item appears in the left nav.
driver.findElements({id: 'menu-item-8462'}).then(function(elements) {
        var isPresent = elements.length;
        if (isPresent){
                console.log('Page element \'menu-item-8462\' (\'RADIOLOGY left-nav link\')) is here');
                elements[0].getText().then(function (text) { console.log('\ttext = ' + text); });
        } else {
                console.log('ERROR: Page element \'menu-item-8462\' (\'RADIOLOGY left-nav link\')) is NOT FOUND');
        }
});
...
driver.findElements({className: 'post-footer'}).then(function(elements) {
	console.log('Checking that the post-footer is present and accurate...');
		...
		require('./includes/footer.js').testPhoneNumber(webdriver, driver, '208-426-1996');
		require('./includes/footer.js').testDeptName(webdriver, driver, 'DEPARTMENT OF RADIOLOGIC SCIENCES');
		require('./includes/footer.js').testEmail(webdriver, driver, 'SUEANTONICH@BOISESTATE.EDU');
		require('./includes/footer.js').testMailingAddress(webdriver, driver, 'HEALTH SCIENCES RIVERSIDE ROOM 126');
        } else {
...
driver.findElements({className: 'entry-title'}).then(function(elements) {
		...
		elements[0].getText().then(function (text) {
	                if (text == 'WELCOME TO THE DEPARTMENT OF RADIOLOGIC SCIENCES') {
       	                	 console.log('\tText is correct: ' + text);
                	} else {
                        	console.log('\tERROR: Text is INCORRECT: ' + text);
				console.log('\t(Should read \'WELCOME TO THE DEPARTMENT OF RADIOLOGIC SCIENCES\'');
                	}
		});
        } else {
