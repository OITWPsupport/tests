...
var URL = 'https://physics.boisestate.edu';
...
// Test that the left nav has a PHYSICS HOME link, and that it has the proper id
driver.findElements({id: 'menu-item-10521'}).then(function(elements) {
        var isPresent = elements.length;
        if (isPresent){
                console.log('Page element of ID \'menu-item-10521\' (the PHYSICS HOME link) is here');
        } else {
                console.log('ERROR: Page element of class \'menu-item-10521\' (the PHYSICS HOME link) is NOT FOUND');
        }
});
...
driver.findElements({className: 'entry-title'}).then(function(elements) {
        ...
        if (isPresent){
                ...
                elements[0].getText().then(function (text) {
                        if (text == 'WELCOME TO THE DEPARTMENT OF PHYSICS') {
                                 console.log('\tText is correct: ' + text);
                        } else {
                                console.log('\tERROR: Text is INCORRECT: ' + text);
                                console.log('\t(Should read \'WELCOME TO THE DEPARTMENT OF PHYSICS\'');
                        }
                });
        } else {
