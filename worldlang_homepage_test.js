...
var URL = 'https://worldlang.boisestate.edu';
...
// Test that a 'WELCOME	' item appears in the left nav.
driver.findElements({id: 'menu-item-14901'}).then(function(elements) {
        var isPresent = elements.length;
        if (isPresent){
                console.log('Page element \'menu-item-14901\' (\'WELCOME left-nav link\')) is here');
                elements[0].getText().then(function (text) { console.log('\ttext = ' + text); });
        } else {
                console.log('ERROR: Page element \'menu-item-14901\' (\'WELCOME left-nav link\')) is NOT FOUND');
        }
});
...
driver.findElements({className: 'post-footer'}).then(function(elements) {
	console.log('Checking that the post-footer is present and accurate...');
		...
		require('./includes/footer.js').testEmail(webdriver, driver, 'DIANACARRILLO@BOISESTATE.EDU');
