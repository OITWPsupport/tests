...
var URL = 'https://camusbibliography.boisestate.edu';
...
// Test that an 'ALBERT CAMUS BIBLIOGRAPHY' item appears in the left nav.
driver.findElements({id: 'menu-item-71'}).then(function(elements) {
        var isPresent = elements.length;
        if (isPresent){
                console.log('Page element \'menu-item-71\' (\'ALBERT CAMUS BIBLIOGRAPHY left-nav link\')) is here');
                elements[0].getText().then(function (text) { console.log('\ttext = ' + text); });
        } else {
                console.log('ERROR: Page element \'menu-item-71\' (\'WELCOME left-nav link\')) is NOT FOUND');
        }
});
...
driver.findElements({className: 'post-footer'}).then(function(elements) {
	console.log('Checking that the post-footer is present and accurate...');
		...
		require('./includes/footer.js').testPhoneNumber(webdriver, driver, 'DEPARTMENT PHONE');
		require('./includes/footer.js').testDeptName(webdriver, driver, 'DEPARTMENT OF MODERN LANGUAGES & LITERATURES');
		require('./includes/footer.js').testEmail(webdriver, driver, ' JASONHERBECK@BOISESTATE.EDU');
		require('./includes/footer.js').testMailingAddress(webdriver, driver, 'DEPARTMENT LOCATION');
        } else { 
...
driver.findElements({className: 'entry-title'}).then(function(elements) {
		...
		elements[0].getText().then(function (text) {
	                if (text == 'ALBERT CAMUS BIBLIOGRAPHY') {
       	                	 console.log('\tText is correct: ' + text);
                	} else {
                        	console.log('\tERROR: Text is INCORRECT: ' + text);
				console.log('\t(Should read \'ALBERT CAMUS BIBLIOGRAPHY\'');
                	}
		});
        } else {
