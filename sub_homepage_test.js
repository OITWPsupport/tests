...
var URL = 'https://sub.boisestate.edu';
...
// Test that the left nav has a STUDENT UNION HOME link, and that it has the proper id
driver.findElements({id: 'menu-item-71'}).then(function(elements) {
        var isPresent = elements.length;
        if (isPresent){
                console.log('Page element of ID \'menu-item-71\' (the STUDENT UNION HOME link) is here');
        } else {
                console.log('ERROR: Page element of class \'menu-item-71\' (the STUDENT UNION HOME link) is NOT FOUND');
        }
});
...
// Test that a 'BUILDING HOURS' item appears in the left nav.
driver.findElements({id: 'menu-item-1061'}).then(function(elements) {
        var isPresent = elements.length;
        if (isPresent){
                console.log('Page element \'menu-item-1061\' (\'BUILDING HOURS left-nav link\') is here');
                elements[0].getText().then(function (text) { console.log('\ttext = ' + text); });
        } else {
                console.log('ERROR: Page element \'menu-item-1061\' (\'BUILDING HOURS left-nav link\') is NOT FOUND');
        }
});
...
driver.findElements({className: 'post-footer'}).then(function(elements) {
        ...
        if (isPresent){
                ...
                require('./includes/footer.js').testPhoneNumber(webdriver, driver, '(208) 426-INFO (4636)');
                require('./includes/footer.js').testDeptName(webdriver, driver, 'STUDENT UNION');
                require('./includes/footer.js').testEmail(webdriver, driver, 'STUDENTUNION@BOISESTATE.EDU');
                require('./includes/footer.js').testMailingAddress(webdriver, driver, 'STUDENT UNION BUILDING');
        } else {
