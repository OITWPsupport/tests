...
var URL = 'https://graduatecollege.boisestate.edu';
...
// Test that the left nav has a FUTURE STUDENTS link, and that it's of the proper class
driver.findElements({id: 'menu-item-6561'}).then(function(elements) {
        var isPresent = elements.length;
        if (isPresent){
                console.log('Page element of ID \'menu-item-6561\' (the FUTURE STUDENTS link) is here');
        } else {
                console.log('ERROR: Page element of class \'menu-item-6561\' (the FUTURE STUDENTS link) is NOT FOUND');
        }
});
...
// Test that a 'ABOUT US' item appears in the left nav: 
driver.findElements({id:'menu-item-32041'}).then(function(elements) {
        var isPresent = elements.length;
        if (isPresent){
                console.log('Page element \'menu-item-32041\' (\'ABOUT US\' left-nav link) is here');
                elements[0].getText().then(function (text) { console.log('\ttext = ' + text); });
        } else {
                console.log('ERROR: Page element \'menu-item-32041\' (\'ABOUT US\' left-nav link) is NOT FOUND');
        }
});
