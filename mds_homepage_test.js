...
var URL = 'https://mds.boisestate.edu';
...
// Test that a 'MULTIDISCIPLINARY STUDIES' item appears in the left nav.
driver.findElements({id: 'menu-item-8722'}).then(function(elements) {
        var isPresent = elements.length;
        if (isPresent){
                console.log('Page element \'menu-item-8722\' (\'MULTIDISCIPLINARY STUDIES left-nav link\') is here');
                elements[0].getText().then(function (text) { console.log('\ttext = ' + text); });
        } else {
                console.log('ERROR: Page element \'menu-item-8722\' (\'MULTIDISCIPLINARY STUDIES left-nav link\') is NOT FOUND');
        }
});
...
driver.findElements({className: 'entry-title'}).then(function(elements) {
        ...
        if (isPresent){
                ...
                elements[0].getText().then(function (text) {
                        if (text == 'MULTIDISCIPLINARY STUDIES') {
                                 console.log('\tText is correct: ' + text);
                        } else {
                                console.log('\tERROR: Text is INCORRECT: ' + text);
                                console.log('\t(Should read \'MULTIDISCIPLINARY STUDIES\'');
                        }
                });
        } else {
