...
var URL = 'https://healthservices.boisestate.edu';
...
// Test that a 'HOME' item appears in the left nav.
driver.findElements({id: 'menu-item-25712'}).then(function(elements) {
        var isPresent = elements.length;
        if (isPresent){
                console.log('Page element \'menu-item-25712\' (\'HOME left-nav link\')) is here');
                elements[0].getText().then(function (text) { console.log('\ttext = ' + text); });
        } else {
                console.log('ERROR: Page element \'menu-item-25712\' (\'HOME left-nav link\')) is NOT FOUND');
        }
});
...
driver.findElements({className: 'post-footer'}).then(function(elements) {
	console.log('Checking that the post-footer is present and accurate...');
		...
		require('./includes/footer.js').testPhoneNumber(webdriver, driver, '(208) 426-1459');
		require('./includes/footer.js').testDeptName(webdriver, driver, 'HEALTH SERVICES');
		require('./includes/footer.js').testEmail(webdriver, driver, 'HEALTHSERVICES@BOISESTATE.EDU');
		require('./includes/footer.js').testMailingAddress(webdriver, driver, 'NORCO BUILDING');
        } else { 
...
driver.findElements({className: 'entry-title'}).then(function(elements) {
		...
		elements[0].getText().then(function (text) {
				...
				console.log('\t(Should read \'HEALTH SERVICES\'');
                	}
		});
        } else {
