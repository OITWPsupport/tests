...
var URL = 'https://secureforms.boisestate.edu/ecampus/onlinemds-9/';
...
driver.findElements({className: 'entry-title'}).then(function(elements) {
		...
		elements[0].getText().then(function (text) {
	                if (text == 'CHECK FOR UNDERSTANDING–ONLINE MULTIDISCIPLINARY STUDIES') {
       	                	 console.log('\tText is correct: ' + text);
                	} else {
                        	console.log('\tERROR: Text is INCORRECT: ' + text);
				console.log('\t(Should read \'CHECK FOR UNDERSTANDING–ONLINE MULTIDISCIPLINARY STUDIES\')');
                	}
		});
        } else {
